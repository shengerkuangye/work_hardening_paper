...
if scanRoot ~= ""
  ...
  assert(all(ismember(["indexed_ti_hex_area_um2"; ...
    "raw_inner_boundary_length_um"; ...
    "retained_inner_boundary_length_um"; ...
    "removed_inner_boundary_length_um"; ...
    "outer_boundary_length_um"; "grain_count"], ...
    string(sensitivity.Properties.VariableNames))));
  ...
  assert(all(sensitivity.raw_inner_boundary_length_um >= ...
  ...
  assert(all(abs(sensitivity.lagb_length_density_um_per_um2 - ...
    sensitivity.lagb_length_um ./ ...
    sensitivity.indexed_ti_hex_area_um2) < 1e-12));
  for sample = expectedSamples'
    ...
    assert(abs(sum(fullDistribution.probability_density_per_degree(rows) .* ...
      fullDistribution.bin_width_deg(rows)) - 1) < 1e-10);
    sensitivityRows = sensitivity.sample == sample;
    assert(nnz(sensitivityRows) == 3);
  end
  expectedFiles = [
